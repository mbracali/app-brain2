...
>
> **Filename**
> - Format: `firstname_lastname.md`, lowercase, ASCII only (strip diacritics: `João` → `joao`). Spaces and punctuation become `_`; collapse repeated `_`.
> - If only a first name is known, use it alone (`joao.md`). If only a nickname is known, use it as-is and record the real name in ALIASES later.
> - **Disambiguation chain** when the filename collides:
>   1. Append normalized FROM, separated by `__`: `joao_silva__company_x.md`
>   2. If still colliding, append an integer: `joao_silva__company_x__2.md`
>
...
>
> **Empty sections** — omit any section that has no data. The template below shows the maximum shape, not the required shape.
